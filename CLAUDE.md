...
│  ├─ services/          前端服务层: tauri.ts (invoke封装), event.ts (事件监听/类型定义), errorHandler.ts
...
│  ├─ src/
│  │  ├─ lib.rs          入口, AppState定义, 命令注册(40+命令), 初始化流程
│  │  ├─ commands/       Tauri命令层 (12个模块): agent, devtools, document, llm, log, session,
│  │  │                       settings, skill, template, update, workspace
│  │  ├─ services/       业务逻辑层
...
│  │  ├─ models/         数据模型: message, session, document, llm, skill, workspace, template, tool
