...
| 前置依赖 | T1.1 |
| 涉及文件 | `src/components/`, `src/hooks/`, `src/stores/`, `src/types/`, `src-tauri/src/llm/`, `src-tauri/src/agent/`, `src-tauri/src/handlers/`, `src-tauri/src/db/`, `src-tauri/src/config/`, `sidecar/`, `shared/` |
| 预估工时 | 1 人天 |
...
### Sprint 3：基础 Handler + 文档处理（第5-6周）

#### T1.11：Handler 接口与注册表

| 属性 | 内容 |
|------|------|
| 任务ID | T1.11 |
| 名称 | Handler 接口与注册表 |
| 描述 | 定义 Handler 的统一接口和注册表机制。Handler 是 Agent 可调用的工具，包括 Rust 原生 Handler 和 Python Sidecar Handler 两种类型。包括：Handler trait 定义（名称、描述、参数 schema、执行方法）、Handler 注册表（注册/查询/列举）、Handler 参数校验（基于 JSON Schema）、Handler 调用结果标准化。 |
| 前置依赖 | T1.2 |
| 涉及文件 | `src-tauri/src/handlers/mod.rs`, `src-tauri/src/handlers/trait.rs`, `src-tauri/src/handlers/registry.rs`, `src-tauri/src/handlers/schema.rs` |
| 预估工时 | 2 人天 |
| 验收标准 | 1) Handler trait 定义清晰，包含 name/description/parameters/execute；2) 注册表支持动态注册和查询；3) 参数校验基于 JSON Schema 可正确拦截非法输入；4) Handler 列表可序列化为 LLM Tool 定义格式 |
...
#### T1.13：generate_document Handler（Word）

| 属性 | 内容 |
|------|------|
| 任务ID | T1.13 |
| 名称 | generate_document Handler（Word） |
| 描述 | 实现基于 python-docx 的 Word 文档生成 Handler。Agent 通过此 Handler 可根据用户描述生成结构化的 Word 文档。包括：文档创建、标题/段落/列表/表格等元素生成、样式应用、文件保存到工作区、生成结果返回（文件路径、页数等元信息）。 |
| 前置依赖 | T1.11, T1.12 |
| 涉及文件 | `sidecar/handlers/generate_document.py`, `sidecar/handlers/docx_utils.py`, `sidecar/requirements.txt` |
| 预估工时 | 2.5 人天 |
| 验收标准 | 1) 可根据 Agent 传入的参数生成 Word 文档；2) 支持标题、段落、列表、表格等常见元素；3) 文档保存到指定工作区目录；4) 返回文件路径和基本元信息；5) 生成的文档可在 Word/WPS 中正常打开 |

#### T1.14：modify_document Handler（Word）

| 属性 | 内容 |
|------|------|
| 任务ID | T1.14 |
| 名称 | modify_document Handler（Word） |
| 描述 | 实现基于 python-docx 的 Word 文档修改 Handler。Agent 通过此 Handler 可对已有文档进行修改操作。包括：文档打开与解析、内容查找与替换、段落插入/删除/移动、样式修改、表格编辑、修改后保存（自动版本备份）。 |
| 前置依赖 | T1.11, T1.12, T1.13 |
| 涉及文件 | `sidecar/handlers/modify_document.py`, `sidecar/handlers/docx_utils.py` |
| 预估工时 | 2.5 人天 |
| 验收标准 | 1) 可打开已有 Word 文档并进行修改；2) 支持内容查找替换、段落操作、样式修改；3) 修改前自动创建版本备份；4) 修改后文档格式不损坏；5) 返回修改摘要信息 |

#### T1.15：read_document Handler

| 属性 | 内容 |
|------|------|
| 任务ID | T1.15 |
| 名称 | read_document Handler |
| 描述 | 实现文档内容读取 Handler，Agent 通过此 Handler 可读取文档内容以了解当前状态。包括：Word 文档内容提取（文本、结构、样式信息）、文档元数据读取（页数、字数、创建时间等）、大文档分段读取、内容摘要生成。 |
| 前置依赖 | T1.11, T1.12 |
| 涉及文件 | `sidecar/handlers/read_document.py`, `sidecar/handlers/docx_utils.py` |
| 预估工时 | 1.5 人天 |
...
#### T2.4：Excel 处理 Handler（openpyxl）

| 属性 | 内容 |
|------|------|
| 任务ID | T2.4 |
| 名称 | Excel 处理 Handler（openpyxl） |
| 描述 | 实现基于 openpyxl 的 Excel 文档处理 Handler，支持生成和修改 Excel 文件。包括：工作簿创建与打开、工作表操作（增删改查）、单元格读写（含公式）、样式设置、图表生成、数据透视表、大文件流式读取。 |
| 前置依赖 | T1.11, T1.12 |
| 涉及文件 | `sidecar/handlers/excel_handler.py`, `sidecar/requirements.txt` |
| 预估工时 | 2.5 人天 |
| 验收标准 | 1) 可生成包含多工作表、公式、样式的 Excel 文件；2) 可修改已有 Excel 文件的内容和样式；3) 大文件（>10MB）可流式读取不超时；4) 生成的文件可在 Excel/WPS 中正常打开 |

#### T2.5：PPT 处理 Handler（python-pptx）

| 属性 | 内容 |
|------|------|
| 任务ID | T2.5 |
| 名称 | PPT 处理 Handler（python-pptx） |
| 描述 | 实现基于 python-pptx 的 PPT 文档处理 Handler，支持生成和修改 PowerPoint 文件。包括：演示文稿创建与打开、幻灯片操作（增删改查）、文本框与形状操作、图片插入、表格插入、母版与布局应用、动画基础支持。 |
| 前置依赖 | T1.11, T1.12 |
| 涉及文件 | `sidecar/handlers/ppt_handler.py`, `sidecar/requirements.txt` |
| 预估工时 | 2.5 人天 |
| 验收标准 | 1) 可生成包含多幻灯片、文本、图片、表格的 PPT 文件；2) 可修改已有 PPT 文件；3) 支持应用母版布局；4) 生成的文件可在 PowerPoint/WPS 中正常打开 |

#### T2.6：PDF 处理 Handler（reportlab/pdfkit）

| 属性 | 内容 |
|------|------|
| 任务ID | T2.6 |
| 名称 | PDF 处理 Handler（reportlab/pdfkit） |
| 描述 | 实现基于 reportlab 和 pdfkit 的 PDF 文档处理 Handler，支持生成 PDF 文件。包括：PDF 文档创建、文本排版（段落、标题、列表）、表格生成、图片嵌入、页眉页脚、页码、目录生成、HTML 转 PDF（pdfkit）。注意：PDF 修改能力有限，主要支持生成。 |
| 前置依赖 | T1.11, T1.12 |
| 涉及文件 | `sidecar/handlers/pdf_handler.py`, `sidecar/requirements.txt` |
| 预估工时 | 2 人天 |
...
#### T2.7：convert_format Handler

| 属性 | 内容 |
|------|------|
| 任务ID | T2.7 |
| 名称 | convert_format Handler |
| 描述 | 实现文档格式转换 Handler，支持不同文档格式之间的转换。包括：Word -> PDF、Excel -> PDF、PPT -> PDF、Markdown -> Word、Markdown -> PDF、HTML -> Word、HTML -> PDF。转换基于已实现的各格式 Handler 组合实现，确保格式保真度。 |
| 前置依赖 | T1.13, T2.4, T2.5, T2.6 |
| 涉及文件 | `sidecar/handlers/convert_format.py`, `sidecar/handlers/converter/` |
| 预估工时 | 3 人天 |
...
| 任务ID | T3.1 |
| 名称 | 自定义 Handler 系统 |
| 描述 | 实现用户自定义 Handler 的能力，允许用户编写和注册自己的 Handler。包括：Handler 模板生成、Handler 编辑器（代码编辑 + 参数定义）、Handler 注册/注销、Handler 沙箱执行（安全隔离）、Handler 分享（导入/导出）、Handler 调试工具。 |
| 前置依赖 | T1.11, T1.12 |
| 涉及文件 | `sidecar/handlers/custom/`, `src-tauri/src/handlers/custom_loader.rs`, `src-tauri/src/handlers/sandbox.rs`, `src/components/HandlerEditor.tsx`, `src/components/HandlerManager.tsx` |
| 预估工时 | 4 人天 |
| 验收标准 | 1) 用户可创建自定义 Handler 并注册到系统；2) 自定义 Handler 可被 Agent 正确调用；3) Handler 执行在沙箱环境中，不影响主进程；4) Handler 可导入/导出为文件；5) Handler 编辑器支持代码高亮和参数定义 |
...
| 前置依赖 | T1.15, T2.11 |
| 涉及文件 | `sidecar/handlers/search.py`, `src-tauri/src/commands/search.rs`, `src/components/SearchPanel.tsx`, `src/components/SearchResults.tsx` |
| 预估工时 | 2 人天 |
...
| 任务ID | T3.6 |
| 名称 | 多工作区切换 |
| 描述 | 实现多工作区管理功能，用户可创建和切换不同的工作区。包括：工作区创建（指定目录）、工作区切换、工作区配置独立（LLM 设置、Handler 配置等）、工作区数据隔离、工作区导入/导出、最近工作区快速切换。 |
...
| 前置依赖 | T1.5, T1.13 |
| 涉及文件 | `src-tauri/src/config/metadata.rs`, `sidecar/handlers/metadata_utils.py`, `src/components/MetadataSettings.tsx` |
| 预估工时 | 1.5 人天 |
...
| 任务ID | T3.9 |
| 名称 | 设置面板完善 |
| 描述 | 完善应用的设置面板，整合所有配置项。包括：LLM 配置（Provider/模型/API Key/参数）、工作区配置、Handler 配置、确认机制配置、外观设置（主题）、快捷键配置、数据管理（导出/清除）、关于页面。 |
| 前置依赖 | T1.5, T3.5 |
| 涉及文件 | `src/components/SettingsPanel.tsx`, `src/components/settings/LLMSettings.tsx`, `src/components/settings/HandlerSettings.tsx`, `src/components/settings/AppearanceSettings.tsx`, `src/components/settings/ShortcutSettings.tsx` |
| 预估工时 | 2.5 人天 |
...
| 任务ID | T4.6 |
| 名称 | 用户文档编写 |
| 描述 | 编写面向用户的帮助文档。包括：快速入门指南、功能说明文档、常见问题 FAQ、快捷键列表、Handler 开发指南、更新日志。文档集成到应用内（帮助面板）和在线文档站点。 |
...
| 6 | T1.13 generate_document Handler | 2.5d | 14d |
| 7 | T1.14 modify_document Handler | 2.5d | 16.5d |
...
| 步骤 | 任务 | 工时 | 累计 |
|------|------|------|------|
| 1 | T2.4 Excel Handler | 2.5d | 2.5d |
...
| Sprint 2 并行组 | T1.7（OpenAI适配器） / T1.9（事件系统） / T1.11（Handler接口） | 后端 3人 |
| Sprint 3 并行组 | T1.13（生成Handler） / T1.15（读取Handler） | Python 2人 |
...
| Sprint 8 并行组 | T3.1（自定义Handler） / T3.2（Prompt模板） / T3.4（搜索） | 后端 1人 + 前端 1人 + Python 1人 |
...
6. 考虑备选方案：将核心 Handler 用 Rust 重写（长期方案）
...
**缓解措施：**
1. 在 Handler 设计中明确支持的范围和限制，不支持的特性给出明确提示
...
| Python 开发 | 1 | Sidecar Handler 开发、文档处理逻辑、格式转换 |
